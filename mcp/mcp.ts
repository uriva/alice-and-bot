...
const setup = async () => {
  ...
  return textResult(
    [
      ...
      "Use aliceandbot_check to poll for incoming messages.",
    ].join("\n"),
  );
};

const check = async () => {
  if (!state || !credentials) return textResult("Run aliceandbot_setup first.");
  const res = await fetch(`${relayUrl}/poll/${state.relayToken}`);
  const { messages } = (await res.json()) as { messages: WebhookUpdate[] };
  if (!messages.length) return textResult("No new messages.");
  const decoded = (
    await Promise.all(
      messages.map(async (msg) => {
        ...
        return { conversationId, text: message.text, type: message.type };
      }),
    )
  ).filter(Boolean);
  return textResult(
...
const server = new Server(
  { name: "aliceandbot", version: "0.1.0" },
...
server.setRequestHandler(ListToolsRequestSchema, () => ({
  tools: [
    ...
    {
      name: "aliceandbot_check",
      description: "Check for new messages from Alice&Bot.",
      inputSchema: { type: "object" as const, properties: {} },
    },
    {
...
server.setRequestHandler(CallToolRequestSchema, (request) => {
  ...
  if (name === "aliceandbot_check") return check();
  ...
  messages: [
    {
      ...
      content: {
        ...
        text: [
          ...
          "Then check for messages with aliceandbot_check and respond with aliceandbot_reply.",
        ].join(" "),
      },
    },
  ],
